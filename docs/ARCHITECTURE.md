...
- **2026-06 — Ask once across sessions (Phase 3).** EIN is promoted to the organization's **natural
  key**, normalized to digits-only, enforced by a partial `UNIQUE(owner_id, ein) WHERE ein IS NOT
  NULL` (migration `0003`) — the promote-on-evidence trigger (invariant 7) since we now need to
  enforce uniqueness to reuse the row. Scoped **per owner**, not global (a global unique would leak a
  tenant's existence via insert failure and fight RLS; swaps to a membership scope when user↔org
  membership lands — same place the RLS `owner_id` check swaps). `CaseStore.create()` is now
  find-or-create: reuse the org by EIN and the plan by `(organization_id, plan_type)`, but always
  insert a new `case` (history powers "most recent answers"). Plan reuse is app-level only — no hard
  constraint, since an org may hold two plans of a type later. New `GET /api/cases/latest` (org- +
  RLS-scoped; `204` when none) feeds the web a **pre-fill + review** flow: known answers seed the
  form, the user confirms/edits (no auto-skip — appropriate for financial data). The raw formatted
